...
def match_multiple(prons, cons, result=[0, True]):
	if len(prons) < len(cons):
		if result[0] == 0: return [0, False]
		else: return result
	if ((is_a_vowel_cmd(cons[0]) and is_a_vowel_cmd(prons[0]) and match_vowel_cmd(cons[0], prons[0])) or (not is_a_vowel_cmd(cons[0]) and not is_a_vowel_cmd(prons[0]) and match_conso_cmd(cons[0], prons[0]))):
		zipp = zipper(prons, cons)
		yes = False
		count = -1
		for i in zipp:
			count += 1
			if not is_a_vowel_cmd(i[0]):
				if not is_a_vowel_cmd(i[1]) and match_conso_cmd(i[0], i[1]): yes = True
				else:
					yes = False
					break
			else:
				if is_a_vowel_cmd(i[0]):
					if match_vowel_cmd(i[0], i[1]): yes = True
					else:
						yes = False
						result[1] = False
						break
